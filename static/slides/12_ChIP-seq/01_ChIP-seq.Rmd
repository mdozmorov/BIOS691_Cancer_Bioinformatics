---
title: "Chromatin Immunoprecipitation ChIP-seq technology"
...
Combine high-throughput sequencing with Chromatin Immunoprecipitation to identify specific protein-DNA interactions genome-wide, including:

- Transcription factors
- Histone marks (various types and modifications)
- RNA Polymerase binding (survey of transcription)
- DNA polymerase binding (investigate DNA replication) 
- DNA repair enzyme binding
...
3. **Immunoprecipitate (IP)**: use an antibody to capture DNA segments with specific proteins.
...
- Often, aligned reads are summarized into "counts" in equal-sized bins genome-wide:
    ...
    2. Count the number of reads started at each bin
...
- Counts from neighboring windows need to be combined to make an inference (so that it’s more robust)
- To combine counts:
...
<!--(a) Main steps of the proposed ChIP-seq processing pipeline. (b) Schematic illustration of ChIP-seq measurements. DNA is fragmented or digested, and fragments crosslinked to the protein of interest are selected with immunoprecipitation. The 5′ ends (squares) of the selected fragments are sequenced, typically forming groups of positive- and negative-strand tags on the two sides of the protected region. The dashed red line illustrates a fragment generated from a long crosslink that may account for the tag patterns observed in CTCF and STAT1 data sets. (c) Tag distribution around a stable NRSF binding position. Vertical lines show the number of tags (right axis) whose 5′ position maps to a given location on positive (red) or negative (blue) strands. Positive and negative values on the y-axis illustrate tags mapping to positive and negative strands, respectively. The solid curves show tag density for each strand (left axis, based on Gaussian kernel with σ = 15 bp). (d) Strand cross-correlation for the NRSF data. The y-axis shows the Pearson linear correlation coefficient between genome-wide profiles of tag density of positive and negative strands, shifted relative to each other by a distance specified on the x-axis. The peak position (red vertical line) indicates a typical distance separating positive- and negative-strand peaks associated with the stable binding positions.-->
...
- Calculate read count at each position (bp) in the genome. Don't use a sliding window
...
Written in Python, runs in the command line. `macs14 -t sample.bed -c control.bed -n result`
...
- Use a dynamic Poisson model to scan the genome and score peaks. 
...
- FDR estimates from sample swapping: flip the IP and control samples and call peaks. The number of peaks detected under each p-value cutoff will be used as null and used to compute FDR
...
- Two-pass analysis. First round - detect possible peak regions by identifying threshold while considering mappability
    - Cut genome into segments ($L=1Mb$). Within each segment, the same number of reads are permuted in a region of $f \times L$, where $f$ is  the proportion of mappable bases in the segment
...
(1) Mapped reads are extended to have the average DNA fragment length (reads on either strand are extended in the 3′ direction relative to that strand) and then accumulated to form a fragment density signal map. (2) Potential binding sites are determined in the first pass of the PeakSeq scoring procedure. The threshold is determined by comparing putative peaks with a simulated segment with the same number of mapped reads. The length of the simulated segment is scaled by the fraction of uniquely mappable starting bases.-->

<!--## Consider mappability: PeakSeq

- Second round analysis
    - Normalize data by counts in background regions
    - Test significance of the peaks identified in the first round by comparing the total count in peak regions with control data, using binomial p-value with Benjamini-Hochberg correction

\begin{center}
\includegraphics[height=120px]{img/peakseq2.png}
\end{center}

(3) After selecting the fraction of potential target sites that should be excluded from the normalization, the scaling factor Pf is determined by linear regression of the ChIP-seq sample against the input-DNA control in 10-Kb bins. Bins that overlap the potential target regions selected for exclusion are not used for regression. The fitted slopes and the Pearson correlations are displayed for Pf set to either 0 or 1. (4) Enrichment and significance are computed for putative binding regions.-->
...
-->
<!--Predicted binding events in two candidate regions in the GABP data. PICS detected one binding event in the region in (a) and two binding events in the region in (b). The observed data of forward and reverse strand aligned reads are shown by ">" and "<" arrowheads, respectively. Vappapbity profis black/white lines, in which the white intervals show nonmappable regions. In (a), the distribution of reverse read positions was biased by a region with low mappability-->
...
\tiny URI - upper ranked identifications
...
- Works fine for some data (H3K4me1/2/3, H3K27ac, H3K36me3)
- Not ideal for H3K9me1: it tends to separate a long block into smaller pieces
...
- Long block requires a bigger smoothing span, which hurts boundary detection
- Data with mixed peak/block (H3K27me3, H3K36me3) requires varied span: the adaptive fitting is computationally infeasible
...
- Has an option for broad peak calling, which uses a post hoc approach to combine nearby peaks.
- Syntax: 
...
- Threshold score value $s_T$ can be determined by requiring the expected number of islands with scores above the threshold $s_T$ to be less than an E-value threshold $e$:
...
- Control sample is important
- Incorporate some special characteristics of the data to improve results
...
- Call peaks from individual datasets
- Union the called peaks to form candidate regions
- Treat the candidate regions as genes, then use the RNA-seq method to test. Or model the differences of normalized counts from two conditions
...
- **ChIPnorm** (Nair et al. 2012, PLoS One): quantile normalization for each data. Ad hoc method for detecting differential peaks
- **DBChIP** (Liang et al. 2012 Bioinformatics) and **DiffBind**: Bioconductor packages, based on RNA-seq method
- **ChIPComp** (Chen et al. 2015 Bioinformatics): Based on the linear model framework, works for general design
...
It is of great interest to study how gene expressions are controlled by protein bindings and epigenetic modifications.
...
- Collect sequences, align, create a consensus sequence by selecting a degeneracy nucleotide sequence for each position
...
- A quantitative score for any DNA sequence can be generated by summing the values corresponding to the observed nucleotide at each position. These scores are proportional to binding energies
...
- Motif finding helps to understand master regulators of gene expression.
